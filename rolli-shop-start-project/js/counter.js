...
function toggleCartStatus(event) { // Отображение надписиси "корзина пуста" и оформление заказа 
  const cartWrapper = document.querySelector(".cart-wrapper"); // Прослушлка на див корзины 

  const cartEmpryBadge = document.querySelector('[data-cart-empty]')
  ...
  if (cartWrapper.children.length > 0) {
    cartEmpryBadge.classList.add('none') // убираем надпись корзина пуста 
    ...
    cartEmpryBadge.classList.remove('none') // добавляем надпись корзина пуста 
